...
if(isset($_GET['Customer_reference'])){
    ...
    $Customer = $db->query("SELECT FirstName FROM Customer WHERE Customer_reference = :Customer_reference", ['Customer_reference' => $_GET['Customer_reference']])->statement->fetch();
...
if(isset($_GET['date'])){
    ...
    $AppointmentHoursDate = $db->query("SELECT AppointmentTime , CustomerID FROM Appointment WHERE AppointmentDate = :date", ['date' => $_GET['date']])->statement->fetchAll();
